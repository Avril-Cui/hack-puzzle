...
const PuzzleGame: React.FC = () => {
  ...
  const handleClick = (tile: Tile) => {
    ...
    const tileCopy = { ...tile, isCover: false }; // ensure isCover is false in queue

    setQueue((prev) => [...prev, tileCopy]);
  ...
  return (
    <div className={styles.gameContainer}>
      <div className={styles.tileBoard}>
        {tiles.map((tile) => {
          const isCovered = tile.isCover || tile.status !== 0;

          return (
            <div
              key={tile.id}
              className={`${styles.tile} ${isCovered ? styles.inactive : ""}`}
              style={{
                left: tile.x,
                top: tile.y,
                zIndex: tile.z,
              ...
              onClick={() => {
                if (!isCovered && queue.length < 7) handleClick(tile);
              }}
            >
