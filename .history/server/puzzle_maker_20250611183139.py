...
def set_initial_cover_state(scene):
    sorted_scene = sorted(scene, key=lambda t: -t['z'])
    occupied = []

    for tile in sorted_scene:
        x1, y1 = tile["x"], tile["y"]
        x2, y2 = x1 + CELL_SIZE, y1 + CELL_SIZE

        is_covered = False
        for ox1, oy1, oz in occupied:
            ox2, oy2 = ox1 + CELL_SIZE, oy1 + CELL_SIZE
            if not (ox2 <= x1 or ox1 >= x2 or oy2 <= y1 or oy1 >= y2):
                is_covered = True
                break

        tile["isCover"] = is_covered

        occupied.append((x1, y1, tile["z"]))
...
def generate_solvable_scene(steps=10, level=3):
    ...
    scene.sort(key=lambda t: t['z'])
    set_initial_cover_state(scene)
    return scene
